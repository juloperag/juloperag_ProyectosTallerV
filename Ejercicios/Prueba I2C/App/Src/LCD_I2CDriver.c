...
void lcd_i2c_gotoxy(I2C_Handler_t *ptrHandlerLCDI2C, uint8_t row, uint8_t column)
{
	...
	delay_ms(10);
	i2c_WriteSingle(ptrHandlerLCDI2C, (ADD | ((lcdAddresses[row][column] & BITS_L)<<4) | LED_ENABLE | EN_ENABLE | RW_SELECT_WRITE | RS_SELECT_ID));   //Enviamos comando y activamos EN
	delay_ms(10);
	i2c_WriteSingle(ptrHandlerLCDI2C, (ADD | ((lcdAddresses[row][column] & BITS_L)<<4) | LED_ENABLE | EN_DISABLED | RW_SELECT_WRITE | RS_SELECT_ID));   //Desactivamos EN
	delay_ms(10);
}
...
void lcd_i2c_putc(I2C_Handler_t *ptrHandlerLCDI2C, char *MsgtoSend)
{
	...
	while(MsgtoSend[i] != '\0')
	{
		...
		delay_ms(20);
		i2c_WriteSingle(ptrHandlerLCDI2C, (((MsgtoSend[i] & BITS_L)<<4) | LED_ENABLE | EN_DISABLED | RW_SELECT_WRITE | RS_SELECT_DR));   //Desactivamos EN
		delay_ms(10);
