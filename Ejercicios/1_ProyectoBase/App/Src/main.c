 ...
 * esta conectada a la salida Clock del Encoder.
*/
GPIO_Handler_t handler_GPIOpinClock =  {0};
EXTI_Config_t handler_EXTIpinClock  = {0};

//Definimos elementos del tipo GPIO_Handler_t leer el estado de la salida Data del Encoder
GPIO_Handler_t handler_Data = {0};

/*Definicion un elemento del tipo EXTI_Config_t y PIO_Handler_t para la interrupcion presente en PC4, la cual
 * esta conectada a la salida SW del Encoder.
*/
GPIO_Handler_t handler_GPIOEncoSW =  {0};
EXTI_Config_t handler_EXTIEncoSW  = {0};

//Definicion de las cabeceras para los tipos de modo a ejecutarse
...
void int_Hardware(void)
{
	...
	//Definicion del pin para el BlinkyPin

	//---------------PIN: PA5----------------
	//Definimos el periferico GPIOx a usar.
	BlinkyPin.pGPIOx = GPIOA;
	//Definimos el pin a utilizar
	BlinkyPin.GPIO_PinConfig.GPIO_PinNumber = PIN_5; 						//PIN_x, 0-15
	//Definimos la configuracion de los registro para el pin seleccionado
	// Orden de elementos: (Struct, Mode, Otyper, Ospeedr, Pupdr, AF)
	GPIO_PIN_Config(&BlinkyPin, GPIO_MODE_OUT, GPIO_OTYPER_PUSHPULL, GPIO_OSPEEDR_MEDIUM, GPIO_PUPDR_NOTHING, AF0);
	...
	//---------------PIN: PB5----------------
		...
		//Definimos el periferico GPIOx a usar.
		handler_Data.pGPIOx = GPIOA;
		//Definimos el pin a utilizar
		handler_Data.GPIO_PinConfig.GPIO_PinNumber = PIN_8; 						//PIN_x, 0-15
		//Definimos la configuracion de los registro para el pin seleccionado
		// Orden de elementos: (Struct, Mode, Otyper, Ospeedr, Pupdr, AF)
		GPIO_PIN_Config(&handler_Data, GPIO_MODE_IN, GPIO_OTYPER_PUSHPULL, GPIO_OSPEEDR_MEDIUM, GPIO_PUPDR_NOTHING, AF0);
		/*Opciones: GPIO_Tipo_x, donde x--->||IN, OUT, ALTFN, ANALOG ||| PUSHPULL, OPENDRAIN |||
		 * ||| LOW, MEDIUM, FAST, HIGH ||| NOTHING, PULLUP, PULLDOWN, RESERVED |||  AFx, 0-15 |||*/
		//Cargamos la configuracion del PIN especifico
		GPIO_Config(&handler_Data);
	...
	//Definimos el periferico GPIOx a usar.
	handler_GPIOpinClock.pGPIOx = GPIOA;
	//Definimos el pin a utilizar
	handler_GPIOpinClock.GPIO_PinConfig.GPIO_PinNumber = PIN_9;
	//Definimos la posicion del elemento pGIOHandler.
	handler_EXTIpinClock.pGPIOHandler = &handler_GPIOpinClock;
	//Definimos el tipo de flanco
	handler_EXTIpinClock.edgeType = EXTERNAL_INTERRUPP_FALLING_EDGE;
	//Cargamos la configuracion del EXTIx
	extInt_Config(&handler_EXTIpinClock);
...
//Definimos la funcion que se desea ejecutar cuando se genera la interrupcion por el EXTI9
...
void modo_numerico(void)
{
	...
	else if (GPIO_RedPin (&handler_Data) == 0)
	{
		//Decremento numerico
		if(unidad>0)
		{
